...
export const commands = {
	...
	async chainSwitchEvent(chain: Chain): Promise<Result<null, string>> {
		try {
			return {
				status: 'ok',
				data: await TAURI_INVOKE('chain_switch_event', { chain }),
			}
		} catch (e) {
			if (e instanceof Error) throw e
			else return { status: 'error', error: e as any }
	...
	async btcDeriveAddress(
		...
		try {
			return {
				status: 'ok',
				data: await TAURI_INVOKE('btc_derive_address', { label, index }),
			}
		} catch (e) {
			if (e instanceof Error) throw e
			else return { status: 'error', error: e as any }
	...
	async btcListDerivedAddresess(): Promise<Result<DerivedAddress[], string>> {
		try {
			return {
				status: 'ok',
				data: await TAURI_INVOKE('btc_list_derived_addresess'),
			}
		} catch (e) {
			if (e instanceof Error) throw e
			else return { status: 'error', error: e as any }
...
export type SyncHeightUpdateEvent = {
...
export type UnlockMsg = {
	ethereum: EthereumUnlock
	bitcoin: BitcoinUnlock
